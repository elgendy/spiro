...
var Spiro;
(function (Spiro) {
    function decorateOptions(options, successFunc, failureFunc) {
        options = options || {
        };
        function decorate(newFunc, funcName) {
            if(newFunc) {
                var oldFunc = options[funcName] || function () {
                };
                options[funcName] = function (originalModel, resp, iOptions) {
                    newFunc(originalModel, resp, iOptions);
                    oldFunc(originalModel, resp, iOptions);
                };
            }
        }
        decorate(successFunc, "success");
        decorate(failureFunc, "error");
        return options;
    }
    ...
    var HateoasModelBase = (function (_super) {
        ...
        HateoasModelBase.prototype.setupOptions = function (options, parent) {
            var success = function (originalModel, resp, iOptions) {
                parent.trigger("requestSucceeded");
            };
            var error = function (originalModel, resp, iOptions) {
                var rs = $.parseJSON(resp.responseText);
                parent.trigger("requestFailed", {
                    statusCode: resp.status,
                    model: new HateoasModelBase(rs)
                });
            };
            return decorateOptions(options, success, error);
        };
        ...
        HateoasModelBase.prototype.fetch = function (options) {
            options = this.setupOptions(options, this);
            if(this.method === "GET") {
    ...
    var ClearMap = (function (_super) {
        ...
        function ClearMap(propertyResource) {
                _super.call(this, {
    }, propertyResource, propertyResource.instanceId(), propertyResource.clearLink());
        }
        return ClearMap;
    })(ArgumentMap);
    ...
    var Links = (function (_super) {
        ...
        Links.WrapLinks = function WrapLinks(links) {
            var ll = new Links();
            ll.add(links);
            return ll;
        }
        ...
        ActionRepresentation.prototype.initParameterMap = function () {
            if(!this.parameterMap) {
                ...
                for(var m in parameters) {
                    var parameter = new Parameter(parameters[m]);
    ...
    var Member = (function (_super) {
        ...
        Member.WrapMember = function WrapMember(toWrap) {
            if(toWrap.memberType === "property") {
                return new PropertyMember(toWrap);
            }
            if(toWrap.memberType === "collection") {
                return new CollectionMember(toWrap);
            }
            if(toWrap.memberType === "action") {
                return new ActionMember(toWrap);
            }
            return null;
        }
        return Member;
    ...
    var PropertyMember = (function (_super) {
        ...
        function PropertyMember(wrapped) {
                _super.call(this, wrapped);
        }
    ...
    var CollectionMember = (function (_super) {
        ...
        function CollectionMember(wrapped) {
                _super.call(this, wrapped);
        }
    ...
    var DomainObjectRepresentation = (function (_super) {
        ...
        DomainObjectRepresentation.prototype.resetMemberMaps = function () {
            ...
            for(var m in members) {
                var member = Member.WrapMember(members[m]);
                this.memberMap[m] = member;
                if(member.memberType() === "property") {
                    this.propertyMemberMap[m] = member;
                } else {
                    if(member.memberType() === "collection") {
